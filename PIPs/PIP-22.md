...
|-------------------|---------------------------------|----------------------|-------------------------------|------------|--------|------------------------------------------|-----------------------|
| 22 | EIP-3074-style Account Abstraction | Implementation of the `AUTH` and `AUTHCALL` EVM opcodes  | Will Schwab (@wschwab), Daniel Gretzke (@gretzke) | [Forum](https://forum.polygon.technology/t/pip-add-eip-3074-style-account-abstraction/12959/1)  | Draft  | Core | 2023-09-22 |
...
The arguments (`yParity`, `r`, `s`) are interpreted as an ECDSA signature on the secp256k1 curve over the message `keccak256(MAGIC || chainId || paddedInvokerAddress || commit)`, where:
 ...
 - `chainId` is the current chain's [EIP-155](https://eips.ethereum.org/EIPS/eip-155) unique identifier padded to 32 bytes.
...
`AUTHCALL` is interpreted the same as `CALL`, except for (note: this list is also the order of precedence for the logical checks):
 ...
 - If the `gas` operand is equal to `0`, the instruction will send all available gas as per [EIP-150](https://eips.ethereum.org/EIPS/eip-150).
...
`AUTHCALL` will not pass more than 63/64th of the available gas for the reasons enumerated in [EIP-150](https://eips.ethereum.org/EIPS/eip-150).
...
![auth message format](../assets/PIP-acct-abstraction-3074/auth-msg.png)

Using `commit` as a hash of values allows for invokers to implement arbitrary constraints. For example, they could allow accounts to have `N` parallel nonces. Or, they could allow a user to commit to multiple calls with a single signature. This would allow mult-tx flows, such as [ERC-20](https://eips.ethereum.org/EIPS/eip-20) `approve`-`transfer` actions, to be condensed into a single transaction with a single signature verification. A commitment to multiple calls would look something like the diagram below.

![multi-call auth message](../assets/PIP-acct-abstraction-3074/auth-msg-multi-call.png)
...
There are no issues with backwards compatibility in the implementation of the `AUTH` and `AUTHCALL` opcodes. 
...
Please see the [Reference Implementation](#reference-implementation) section for the various implementations of the different facets of [EIP-3074](https://eips.ethereum.org/EIPS/eip-3074). Some provide test cases.
